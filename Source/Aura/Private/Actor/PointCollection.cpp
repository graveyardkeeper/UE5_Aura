...
TArray<USceneComponent*> APointCollection::GetGroundPoints(int32 NumPoints, float YawOverride)
{
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		if (Pt != Pt_0)
		{
			// 中心点位置固定为中心，其他点按照YawOverride旋转
			FVector ToPoint = Pt->GetComponentLocation() - Pt_0->GetComponentLocation();
			ToPoint = ToPoint.RotateAngleAxis(YawOverride, FVector::UpVector);
			Pt->SetWorldLocation(Pt_0->GetComponentLocation() + ToPoint);
		}

		// 中心点在斜面上时，将点紧贴地面而不是空中，忽略敌人和主角
		...
		Pt->SetWorldRotation(UKismetMathLibrary::MakeRotFromZ(HitResult.ImpactNormal)); // 法线是与地面垂直的，设置Pt的Z轴始终与法线方向一致
